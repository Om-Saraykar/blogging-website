...
const Article = () => {
  ...
  useEffect(() => {
    const fetchArticleAndComments = async () => {
      try {
        ...
        // Increment views
        await axios.put(`${API_BASE_URL}/posts/${id}/increment-views`, {}, {
          headers: {
        ...
        setComments(commentsResponse.data); // Set the comments from the response
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/comments/${id}`, { content: comment }, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
  ...
  return (
    <div className="p-8 text-black w-[80%] mx-auto mt-[70px] rounded bg-white">
      {isEditing ? (
